...
const InstitutionLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${baseUrl}/api/institutions/login`, {
      ...
      // Store token, institution_id, and username in localStorage
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem('institutionNameId', user.institution_id);
      localStorage.setItem('institutionUsername', user.username);
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div className="relative" ref={dropdownRef}>
              ...
              <div
                className="relative block w-full px-4 py-3 border border-gray-300 text-gray-900 rounded-md bg-white hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition duration-200 ease-in-out text-base cursor-pointer"
                ...
                <span className={institution ? 'text-gray-900' : 'text-gray-500'}>
                  {loadingInstitutions ? (
                    <div className="flex items-center gap-2">
                      <Loader className="h-5 w-5" color="text-orange-600" />
              ...
              {isDropdownOpen && !loadingInstitutions && (
                <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                  {institutions.map((inst, index) => (
                    <div
                      key={inst.id || index}
                      className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-orange-50 hover:text-orange-600"
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                className="appearance-none relative block w-full px-4 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md bg-white hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition duration-200 ease-in-out text-base"
          ...
          <div className="space-y-4">
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-base font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition duration-200 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={loading}
              className="group relative w-full flex justify-center py-3 px-4 border border-orange-600 text-base font-medium rounded-md text-orange-600 bg-white hover:bg-orange-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition duration-200 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Sign in as Guest
            </button>
          </div>
